...
evals |>
  mutate(seniority = as.double(seniority),
         SEN_level = if_else(seniority <= 4,
                             "junior",
                             "senior")) |>
  group_by(sex, SEN_level, academic_degree) |>
  summarize(distinct_vals = n_distinct(teacher_id)) |>
  ungroup() |>
  pivot_longer(cols = c(sex, SEN_level, academic_degree)) |>
  group_by(value) |>
  summarize(num = sum(distinct_vals)) |>
  pivot_wider(names_from = value,
              values_from = num) |>
  select(female, male, junior, senior, no_dgr, ma, dr, prof) |>
  rename(Female = female,
         Male = male,
         "Junior (4 years of less)" = junior,
